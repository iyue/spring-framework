...
of the `xin.iyue.Main` class has a reference to the proxy. This means that method calls on that
...
----
	...
	public final class xin.iyue.Main {

		public static void main(String[] args) {
			ApplicationContext ctx = new ClassPathXmlApplicationContext("beans.xml", xin.iyue.Main.class);
...
We use the following command to run the `xin.iyue.Main` class shown earlier:

====
[literal]
[subs="verbatim,quotes"]
----
java -javaagent:C:/projects/foo/lib/global/spring-instrument.jar foo.xin.iyue.Main
----
====
...
The output from the execution of the `xin.iyue.Main` program looks something like the next example.
...
Spring beans. The following slight variation on the `xin.iyue.Main` program yields the same
result:
...
----
	...
	public final class xin.iyue.Main {
		...
		public static void main(String[] args) {
			new ClassPathXmlApplicationContext("beans.xml", xin.iyue.Main.class);
